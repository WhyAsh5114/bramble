...
## Gap 3 — the local password (`WALLET_PASS`) has no setup command, and no recovery path
...
**This turned out to be a real, not just theoretical, gap.** `ring init` on Sept 10 used an ad hoc dev password, entered once as a shell env var and never written down — a completely ordinary way to end up choosing a password under time pressure. Days later, running `ring encrypt` for real, nobody could remember it. There's no recovery path: the OS keychain entry `wallet-cli` itself writes (`security find-generic-password -a default -s ledger-wallet-cli -w`, the exact command the CLI's own error message suggests) doesn't exist under that account name at all — what's actually there is an item named `member-private-key-<id>`, and that item is *encrypted by* `WALLET_PASS`, not a copy of it. So the keychain, despite holding real ring state, offers no way to recover a forgotten password; the only way back in is `ring destroy` + `ring init` with a new one, discarding anything encrypted under the old password (per this gate's own "Known limitation" above — the mechanism is symmetric between forgetting a password and a Trust Chain membership rotation, but only one of those is externally forced). We ultimately recovered the actual password by grepping our own AI coding assistant's saved session transcripts for the literal command that had been run three days earlier — not something available to a developer without that specific kind of incidental logging.

**Suggestion:** at minimum, have `ring init` print an explicit, impossible-to-miss warning that this password has no recovery mechanism and losing it means losing everything encrypted so far — the current flow gives it exactly the same weight as any other CLI prompt. A password-manager-friendly generation option (`ring init --generate-password`, printing a copyable value once) would also help, since the natural failure mode here is a developer typing something quick and forgettable specifically because nothing signals the stakes.
...
**Impact for this project specifically:** the intended demo shot — a human approving an agent's widened permission with one physical button press, the actual on-chain grant, live on Sepolia — wasn't achievable with shipped `wallet-cli`. With a day of runway left we built our own signer directly against `@ledgerhq/hw-app-eth` instead (bypassing `wallet-cli` entirely for the broadcast step — see Gap 5), so the button-press demo is real after all; it just isn't something `wallet-cli` itself could do.
...
## Gap 5 — `hw-app-eth`'s `signTransaction` return value silently changes meaning by transaction type

Building our own signer against `@ledgerhq/hw-app-eth` (not `wallet-cli` — see Gap 4's impact note) directly, `eth.signTransaction(path, rawTx, resolution)` returns `{ v, r, s }` for every transaction type, with no type tag on the response itself. For a legacy transaction, `v` is the familiar EIP-155-encoded value. For an EIP-1559 (type 2) transaction — the only kind we ever sent — `v` is actually the *yParity* (`0` or `1`), a completely different value with the same field name. Nothing in the method's TypeScript types or JSDoc says this changes by transaction type; we only found it by testing (a real signed transaction that serializes and looks fine but recovers to the wrong address the moment you check).

**Why this is a real risk, not a nitpick:** the failure mode is silent. A wrong `v`/`yParity` interpretation still produces a validly-shaped signed transaction — RLP encoding doesn't care whether a value is semantically correct. It would have shipped to `eth_sendRawTransaction` and either reverted with a confusing signature-related error, or (worse, if the wrong interpretation happened to recover to a real but wrong address with insufficient funds/nonce) failed in a way that looks like an account problem, not a signing bug. We caught it only because we locally recovered the signer address from the assembled signature *before* ever broadcasting, specifically because we didn't trust this field going in.

**Suggestions:**
- Return an explicit `yParity` field for type-2+ transactions instead of overloading `v`, or at minimum document the overload directly on `signTransaction`'s type signature, not just in changelogs/migration notes elsewhere.
- Ship a code example for EIP-1559 specifically in the package README — the current example (if any) reads as if `v` is always ready to use as-is with common serialization libraries (viem, ethers), which is exactly what's not true for this transaction type.
- Consider a fourth field in the response — `txType` or similar — cheap to add, removes the ambiguity for every caller rather than each one having to learn it the way we did.
