...
A new device is enrolled by a real physical button press on a genuine Nano S Plus: four transactions (deploying its resolver, registering it, publishing its key, allocating its mesh-ip — the fourth added Sept 12 alongside automatic mesh addressing, `docs/adr/0009-mesh-ip-allocation.md`), four on-device confirmations, no software key ever signs on the device's behalf. `wallet-cli send` itself doesn't support Sepolia or other testnets/L2s as of v2.1.0 (confirmed against Ledger's own `agent-skills` repo — see `docs/12_LEDGER_DX_FEEDBACK.md` Gap 4), so we built our own signer directly against `@ledgerhq/hw-app-eth` (`admincli/src/ledger-eth-sign.ts`) rather than accept a software-signed enrollment.

Separately, the agent story: it starts with narrow access and its wider request is denied. The administrator's ACL-granting key is never stored as plaintext — it's encrypted at rest under a Ledger Key Ring (LKRP), rooted to the same device at provisioning time, decrypted in memory per grant, headlessly, on that machine. That decrypted key is used only to compute the ECDH capability digest the grant publishes (`docs/adr/0005`); the on-chain write that actually publishes it is signed by a separate, ordinary operator key on the same machine, not the Ledger. Key Ring protects the granter's secret at rest on the provisioned admin machine; it does not give an unprovisioned host a decrypt path or require a per-grant button press. Widening the agent's scope, and its subsequent success at the previously-denied task, both happen live, without a restart. The agent never receives the private backend's API credential.
...
Bramble does not yet implement STUN, automatic NAT-failure detection, or production relay authentication. Relay sessions are short-lived and do not handle NAT remapping. Node and Hedera payer keys are currently software keys. Revocation timing depends on the ENS polling interval and resolver availability. Mesh addressing is auto-assigned at enroll time and peer discovery is full-mesh-by-default when `-peer` is omitted (`docs/adr/0009-mesh-ip-allocation.md`, `docs/adr/0010-full-mesh-peer-discovery.md`) — this only affects WireGuard-layer admission, not service access, which ACL still gates per request. The default public Sepolia RPC endpoint showed real, multi-thousand-block sync lag between backends during live verification; a demo recording should **export** `SEPOLIA_RPC_URL` to a dedicated endpoint before starting admin commands or nodes rather than rely on the default or assume the admin CLI's `.env` loader updates an already-imported RPC constant.
