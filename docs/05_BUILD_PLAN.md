...
**✅ Failover mitigation built Sept 10, 2026** — `brambled/rendezvous.ExchangeAny` tries every rendezvous relay `brambled` knows about (discovered from the relay registry, or `-rendezvous` overrides) in sequence, splitting the timeout budget across them so one unreachable relay can't make the whole attempt take len(set)×timeout. See `docs/adr/0003`'s Consequence section.
...
- **Section C (built, live run pending)** — data-plane relay metering, `docs/adr/0008-data-relay-metering.md`. One ephemeral UDP port per purchased session (`relay/datarelay.go`), priced per requested byte via `@x402/hono`'s `DynamicPrice` (`relay-sidecar/src/routes/datarelay.ts`, `pricing.ts`); `brambled` publishes the relay's address in place of its own for any `-relay-peer`-flagged peer (`brambled/main.go`'s `relayRoutedEndpoint`), and both sides arm a persistent WireGuard keepalive so the relay learns each side's address even though only one side initiates the real handshake. Completes Gate 4.2 pending a live run.
- **Section D (built, live run pending)** — `brambled` picks among a pool of data-relay candidates by price+latency (`brambled/datarelay.Pick`), querying each candidate's `relay-sidecar` `GET /price`. Kill-mid-transfer failover, closed Sept 10, 2026: `brambled/datarelay.WaitForStall` detects a dead relay via `wgnode.PeerInfo.RxBytes` staleness (not handshake staleness — a live session's key stays valid for minutes without a fresh handshake, so bytes-stopped-moving is the faster, more direct signal), and `brambled/main.go`'s `watchDataRelayFailover` re-points the peer at a fresh relay from the remaining pool. Real end-to-end test, not a state-flag check: `TestDataRelayFailover_SwitchesToBackupRelayWhenPrimaryStalls` kills a real UDP relay mid-ping-loop and confirms a real WireGuard handshake recovers through the backup. Full design: `docs/adr/0008`'s "Kill-mid-transfer failover" section. Completes Gate 4.3, pending the live run.
...
**🔧 BUILT, live run pending, Sept 10 2026** — selection (`brambled/datarelay.Pick`, unit tested in `select_test.go`) and kill-mid-transfer failover (`brambled/datarelay.WaitForStall` + `brambled/main.go`'s `watchDataRelayFailover`, real end-to-end test in `TestDataRelayFailover_SwitchesToBackupRelayWhenPrimaryStalls` — kills a real UDP relay mid-transfer, confirms real WireGuard recovery through the backup) are both built and passing. Full design in `docs/adr/0008`'s "Kill-mid-transfer failover" section. Not yet run against live Hedera testnet accounts with two real relay-sidecars — that live run is what flips this to ✅ VERIFIED.
