...
- **Section D (selection built, live run pending; failover deferred)** — `brambled` picks among a pool of data-relay candidates by price+latency (`brambled/datarelay.Pick`), querying each candidate's `relay-sidecar` `GET /price`. Kill-mid-transfer failover is explicitly out of scope this pass (needs a liveness watcher on `wgnode.Node.Peers()`'s handshake staleness — see ADR 0008's "Explicitly out of scope"; cut-order item #2 already covers this). Starts Gate 4.3.
...
**🔧 BUILT (selection only), live run pending, Sept 9 2026** — `brambled/datarelay.Pick` (unit tested, `brambled/datarelay/select_test.go`) picks the cheaper of a pool of `-data-relay` candidates by price+latency. Kill-mid-transfer failover is not built this pass (see Section D's note above) — per the cut order, this gate can ship as "selection demoed with one relay, failover explicitly named as future work" if the schedule doesn't allow closing the gap.
