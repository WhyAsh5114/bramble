...
| Hardware signer | `@ledgerhq/wallet-cli` **v2.1.0** (installed and checked Sept 7 — supersedes the earlier v1.0.1 pin) | Install `npm i -g @ledgerhq/wallet-cli`. The "flags may change" warning was correct: `send` now takes `--data` (0x-prefixed EVM calldata) alongside `--to`/`--amount`, and there's a first-party `skill install --agent claude` command. `send --help` and `ring --help` output is JSON now, not plain text — parse accordingly if scripting around it |
...
| Payments | `@x402/core`, `@x402/hedera`, `@x402/fetch` (client), `@x402/hono` (server) — all v2.25.0, confirmed real and working Sept 9 (Gate 0.4) | v1 packages (`x402-express` etc.) are deprecated. `@x402/fetch`/`@x402/hono` wrap the lower-level `x402Client`/`x402ResourceServer` and are the right layer to build on — verified far less code than hand-rolling the HTTP glue |
| Hedera SDK | `@hiero-ledger/sdk` (^2.87.0) | `@x402/hedera` re-exports a pinned subset (`AccountId`, `Client`, `Hbar`, `PrivateKey`, `TokenAssociateTransaction`, `TokenId`, `TransferTransaction`, etc.) — import from `@x402/hedera` instead of installing this directly alongside it, or duplicate installs can throw at runtime (`t.startsWith is not a function`), per that package's own README |
| Facilitator | **Blocky402**, hosted testnet instance `https://api.testnet.blocky402.com` (confirmed live, no API key needed on testnet) | Required by the Hedera track. Network id on the wire: `"hedera:testnet"` |
| Chain (payments) | Hedera testnet | Settlement asset is a choice, not forced: native HBAR (`asset: "0.0.0"`, 8 decimals/tinybar, no association needed) or testnet USDC (`0.0.429274`, 6 decimals, HTS — **watch token association**, no EVM analogue). Gate 0.4's proof uses HBAR specifically to avoid the association step; Section B/C may choose differently for the "HTS tokens... in the settlement path" extra-points bullet |
