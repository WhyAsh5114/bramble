...
- Phase 4 (`05_BUILD_PLAN.md`) needs a small ENS record (or record set) per tailnet listing its rendezvous relay(s), separate from however data-plane relays advertise themselves for price/latency discovery. This is additive to the existing schema, not a change to Gate 1.4's no-endpoints-on-device-subnames rule — rendezvous relay records live on the relay's own subname, same as data-plane relay records do.
...
- A tailnet's published rendezvous set needs at least two relays, and the node agent's candidate-exchange logic needs failover across the set (see "Trust boundary" above) — this is a Gate 1.2 concern, not just a Phase 4 one, since it affects connectivity resilience from the first connection attempt onward.
