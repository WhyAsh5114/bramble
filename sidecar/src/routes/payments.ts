...
// see index.ts) once per relay it's about to call rendezvous.Exchange
// against — sidecarUrl identifies which relay's own payment sidecar to pay,
// since a token only verifies at the relay whose secret minted it (see
// payments/client.ts's doc comment). The real x402/Hedera payment happens
// inside payForRendezvousToken; this route is just the local HTTP boundary
// brambled already crosses for everything else (docs/adr/0002, generalized
// to payments by docs/adr/0007).
paymentsRoute.post('/rendezvous-token', async (c) => {
  const { sidecarUrl } = await c.req.json<{ sidecarUrl: string }>()
  try {
    const result = await payForRendezvousToken(sidecarUrl)
