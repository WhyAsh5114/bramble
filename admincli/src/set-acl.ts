...
// Requires BRAMBLE_GRANTER_PRIVATE_KEY in .env: the granter identity's own
// WireGuard private key (bare hex, no 0x) — the same kind of key any
// enrolled device already has, not a new secret type.
import { createWalletClient, http, toHex } from 'viem'
import { normalize, packetToBytes } from 'viem/ens'
import { x25519 } from '@noble/curves/ed25519'
import { bytesToHex, hexToBytes } from '@noble/hashes/utils'
import { RPC_URL, tailnetName } from '../../sidecar/src/ens/config'
import { accountFromEnv, hackathonSepolia, publicClient, wireguardPrivateKeyFromEnv } from './setup'
...
async function main() {
  ...
  const granterPrivKeyHex = wireguardPrivateKeyFromEnv('BRAMBLE_GRANTER_PRIVATE_KEY')
